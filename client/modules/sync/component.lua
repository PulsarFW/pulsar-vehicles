	...
	Sync = {
		...
		Neons = {
			...
			IsDisabled = function(self)
				if SYNC_DRIVING_VEHICLE and plsr.Vehicles.Sync.Neons:Has() then
					local currentState = SYNCED_VEHICLES[SYNC_DRIVING_VEHICLE]
					if currentState then
			...
			Toggle = function(self, toggle)
				if SYNC_DRIVING_VEHICLE and plsr.Vehicles.Sync.Neons:Has() and CheckActionRateLimit("neons", 20) and not plsr.Police:IsPdCar(SYNC_DRIVING_VEHICLE) then
					local currentState = SYNCED_VEHICLES[SYNC_DRIVING_VEHICLE]
					if currentState then
						if not toggle then
...
AddEventHandler("Proxy:Shared:ExtendReady", function(component)
	if component == "Vehicles" then
		exports["pulsar_core"]:ExtendComponent(component, _vehicleSyncStuff)
	end
end)
